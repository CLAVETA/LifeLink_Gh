...
async def _login(data):
    ...
    if response.status_code == 200:
        ...
        return ui.navigate.to("/volunteer/dashboard")


@ui.page("/volunteer/login")
        ...
        with ui.element("section").classes("flex-grow flex items-center justify-center w-full px-4 md:mt-5"):
            with ui.card().classes("w-full md:w-[60%] lg:w-[40%] p-6 bg-white shadow-md items-center rounded-md"):
                ...
                with ui.element("div").classes("flex flex-col w-full pt-5 pb-2 text-gray-700"):
                    ui.label("volunteer Name").classes("text-sm text-left")
                    ui.input(placeholder="Your Name").props("flat outlined dense color=red-100").classes("rounded-sm bg-white text-xs")
                ...
                with ui.element("div").classes("flex flex-col w-full py-2 text-gray-700"):
                    ...
                    _login_btn = (ui.button("Login as volunteer",on_click=lambda:_login(data={"email": email.value,"password": password.value })).props("no-caps flat dense").classes("bg-red-600 text-white hover:bg-red-500 rounded-md my-4 py-2 px-4"))
                    
                    # Sign up link
                    with ui.row().classes("items-center justify-center space-x-1 gap-0 m-0 p-0"):
                        ui.label("Don't have an account?")
                        ui.link("Sign up", "/donor_registration").classes("no-underline text-red-600")
