...
async def _login(data):
    ...
    if response.status_code == 200:
        response.json()
        token = response.json().get("access_token")  # or however you extract it
        if token:
            app.storage.user["access_token"] = token  # store token for other pages
            print("Stored token:", app.storage.user.get("access_token"))
            ui.notify("✅ Logged in", color="green")
        else:
            ui.notify("⚠️ Login succeeded but token missing", color="red")
    ...
    with ui.element("main").classes("min-h-screen w-full flex flex-col"):
        ...
        with ui.row().classes(
            ...
            with ui.row().classes("gap-3 mt-3 md:mt-0"):
                ui.button("Donor Login").props("no-caps flat dense").classes(
                    "bg-red-600 text-white hover:bg-red-500 rounded-md px-4"
                )
        # Signin form
        with ui.element("section").classes(
            "flex-grow flex items-center justify-center w-full px-4 md:mt-5"
        ):
            with ui.card().classes(
                "w-full md:w-[60%] lg:w-[40%] p-6 bg-white shadow-md items-center rounded-md"
            ):
                ...
                with ui.element("div").classes(
                    "flex flex-col w-full pt-5 pb-2 text-gray-700"
                ):
                    ui.label("Donor Name").classes("text-sm text-left")
                    ui.input(placeholder="Your Name").props(
                        "flat outlined dense"
                    ).classes("rounded-sm bg-white text-xs")
                ...
                with ui.element("div").classes(
                    "flex flex-col w-full py-2 text-gray-700"
                ):
                    ...
                    _login_btn = (
                        ...
                        .classes(
                            "bg-red-600 text-white hover:bg-red-500 rounded-md my-4 py-2 px-4"
                        )
                    )

                    # Sign up link
                    with ui.row().classes(
                        "items-center justify-center space-x-1 gap-0 m-0 p-0"
                    ):
                        ui.label("Don't have an account?")
                        ui.link("Sign up", "/donor_registration").classes(
                            "no-underline text-red-600"
                        )

        # Footer
        with ui.row().classes(
            "flex flex-col md:flex-row items-center justify-center px-7 w-full bg-gray-50 py-4 text-sm md:mt-5 text-gray-700"
        ):
