...
def donor_sidebar(auth_token: str = None):
    ...
    if auth_token:
        try:
            ...
            if response.status_code == 200:
                ...
                # Parse donor data safely
                donor_info["name"] = data.get("full_name", "Donor").upper()
                donor_info["blood_type"] = data.get("blood_type", "Unknown")
                donor_info["location"] = data.get("location", "N/A")
                donor_info["donor_id"] = str(data.get("donor_id", "0000"))
                donor_info["is_available"] = bool(data.get("is_available", False))

            else:
                ui.notify(
                    f"Failed to load donor profile (Status: {response.status_code})",
                    color="red",
                )
    ...
    # Refreshable label for availability
    @ui.refreshable
    def availability_status_text(is_available):
        status_text = "Available" if is_available else "Unavailable"
        color = "text-green-600" if is_available else "text-gray-600"
        ui.label(status_text).classes(f"text-lg font-medium {color}")
    ...
    def handle_availability_toggle(e):
        ...
        ui.notify(f"Updating availability to: {new_status} (API sync pending...)", color="info")

    # Donor card dialog
    def show_donor_card():
        with ui.dialog() as dialog, ui.card().classes(
            "p-6 w-full max-w-md rounded-2xl shadow-2xl bg-white"
        ):
            with ui.column().classes("items-center space-y-4"):
                ...
                with ui.card().classes(
                    "w-90 items-center border border-gray-300 rounded-xl p-4 shadow-md bg-white"
                ):
                    ui.image(donor_info["profile_picture_url"]).classes(
                        "w-20 h-20 rounded-full object-cover border-2 border-red-600"
                    )
                    ...
                    with ui.row().classes("justify-between items-center w-full"):
                        ...
                        with ui.column().classes("items-center"):
                            ...
                            ui.label(f"ID: {donor_info['donor_id']}").classes(
