    ...
    with ui.element("main").classes("min-h-screen w-full flex flex-col"):
        ...
        with ui.row().classes("flex flex-col md:flex-row justify-center gap-6 w-full px-4 py-6"):
            with ui.card().classes("w-full md:w-1/4 p-6 bg-white shadow-md rounded-md border border-red-100"):
                ...
                hospital_map = ui.column().classes("w-full h-50 rounded-md overflow-hidden")
                with hospital_map:
                    ui.html(
                        """
                        <iframe
            ...
            with ui.card().classes("w-full md:w-[65%] p-6 bg-white shadow-md rounded-md border border-red-100"):
                        ...
                        if response.status_code == 200:
                                ...
                                if token_payload:
                                    try:
                                        ...
                                        if len(parts) >= 2:
                                            ...
                                            for key in ("hospital_id", "hospitalId", "hospital", "id", "sub", "user_id", "userId"):
                            ...
                            if hospital_id:
                                def _req_hospital_id(r):
                                    # direct keys
                                    for k in ("hospital_id", "hospitalId", "hospital_id"):
                                        if k in r:
                                            return r.get(k)
                                    # nested object
                                    h = r.get("hospital")
                                ...
                                data = [r for r in data if str(_req_hospital_id(r)) == str(hospital_id)]
                            ...
                            for req in data:
                                with request_table:
                                    ...
                                    with ui.row().classes(
                                        ...
                                        ).classes("text-center no-underline text-gray-900")
                                        ...
                                        with ui.element("div").classes("flex items-center justify-center whitespace-nowrap gap-2"):
                                            ui.label(date_str or "N/A").classes("text-center whitespace-nowrap")

                                            with ui.button(icon='more_vert').props('flat dense').classes('text-red p-0'):
                                                with ui.menu():
                                                    ...
                                                    async def _on_edit(_ev=None, r=req):
                                                        ...
                                                        dlg = ui.dialog()
                                                        with dlg:
                                                            with ui.card().classes('w-[90vw] md:w-[25vw] p-4'):
                                                                ui.label('Edit Request').classes('text-lg font-semibold mb-1 text-center')
                                                                ui.label('Blood Type').classes('text-sm text-left')
                                                                bt = ui.select(
                                                                    ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"],
                                                                    value=r.get('blood_type', 'A+'),
                                                                ).props('outlined dense').classes('bg-red-50 text-xs w-full')
                                                                ui.label('Quantity').classes('text-sm text-left')
                                                                qty = ui.input(value=str(r.get('quantity', ''))).props('flat outlined dense').classes('bg-red-50 text-xs w-full')
                                                                ui.label('Patient Condition').classes('text-sm text-left')
                                                                cond = ui.textarea(value=r.get('patient_condition', '')).props('outlined').classes('bg-red-50 text-xs w-full')
                                                                with ui.row().classes('justify-end gap-2 mt-3'):
                                                                    async def _submit_edit(_ev=None):
                                                                        # add immediate feedback
                                                                        if not token:
                                                                            ui.notify('You must be logged in to edit a request', type='warning')
                                                                            return
                                                                        save_btn.props(add='disable loading')
                                                                        payload = {
                                                                        ...
                                                                        def _put():
                                                                            return requests.put(f"{base_url}/requests/{r.get('id')}", data=payload, headers={'Authorization': f'Bearer {token}'})

                                                                        try:
                                                                            resp = await run.io_bound(_put)
                                                                        except Exception as ex:
                                                                        ...
                                                                        if resp.status_code in (200, 201):
                                                                            ...
                                                                            try:
                                                                                ...
                                                                                msg = resp.text
                                                                            ui.notify(f'Failed to update: {resp.status_code} {msg}', type='warning')

                                                                    save_btn = ui.button('Save', on_click=_submit_edit).props('flat dense').classes('bg-red-600 text-white')
                                                                    ui.button('Cancel', on_click=lambda: dlg.close()).props('flat dense')
                                                        dlg.open()

                                                    ui.menu_item('Edit Request', on_click=_on_edit).props('icon=edit')

                                                    # Delete request: ask for confirmation then DELETE /requests/{id}
                                                    async def _on_delete(_ev=None, r=req):
                                                        token = app.storage.user.get('access_token')
                                                        dlg = ui.dialog()
                                                        with dlg:
                                                            with ui.card().classes('p-4 w-[90vw] md:w-[30vw]'):
                                                                ui.label('Delete Request').classes('text-lg font-semibold')
                                                                ui.label('Are you sure you want to delete this request? This action cannot be undone.').classes('text-sm text-gray-700 my-3')
                                                                with ui.row().classes('justify-end gap-2'):
                                                                    async def _confirm_delete(_ev=None):
                                                                        if not token:
                                                                            ui.notify('You must be logged in to delete a request', type='warning')
                                                                            return
                                                                        delete_btn.props(add='disable loading')
                                                                        def _del():
                                                                            return requests.delete(f"{base_url}/requests/{r.get('id')}", headers={'Authorization': f'Bearer {token}'})

                                                                        try:
                                                                            resp = await run.io_bound(_del)
                                                                        except Exception as ex:
                                                                            ui.notify(f'Network error: {ex}', type='negative')
                                                                            delete_btn.props(remove='disable loading')
                                                                            return

                                                                        delete_btn.props(remove='disable loading')
                                                                        if resp.status_code in (200, 204):
                                                                            ui.notify('Request deleted', color='positive')
                                                                            dlg.close()
                                                                            await load_requests()
                                                                        elif resp.status_code == 401:
                                                                            ui.notify('Unauthorized. Please log in again.', type='warning')
                                                                            ...
                                                                            try:
                                                                                ...
                                                                                msg = resp.text
                                                                            ui.notify(f'Failed to delete: {resp.status_code} {msg}', type='warning')

                                                                    delete_btn = ui.button('Delete', on_click=_confirm_delete).props('flat dense').classes('bg-red-600 text-white')
                                                                    ui.button('Cancel', on_click=lambda: dlg.close()).props('flat dense')
                                                        dlg.open()

                                                    ui.menu_item('Delete Request', on_click=_on_delete).props('icon=delete')

                                                    # Confirm donation: open responses modal where user can POST to confirm
                                                    async def _on_confirm(_ev=None, r=req):
                                                        token = app.storage.user.get('access_token')
                                                        dlg = ui.dialog()
                                                        with dlg:
                                                            with ui.card().classes('p-4 w-[95vw] md:w-[60vw]'):
                                                                ui.label('Responses').classes('text-lg font-semibold')
                                                                list_container = ui.column().classes('max-h-60 overflow-auto my-2')

                                                                async def _load_responses():
                                                                    if not token:
                                                                        ui.notify('You must be logged in to view responses', type='warning')
                                                                        return
                                                                    get_btn.props(add='disable loading')
                                                                    def _get():
                                                                        return requests.get(f"{base_url}/requests/{r.get('id')}/responses", headers={'Authorization': f'Bearer {token}'})
                                                                    try:
                                                                        resp = await run.io_bound(_get)
                                                                    except Exception as ex:
                                                                        ui.notify(f'Network error: {ex}', type='negative')
                                                                ...
                                                                get_btn = ui.button('Refresh', on_click=lambda: _load_responses()).props('flat dense').classes('ml-0')
                                                                await _load_responses()
                                                                with ui.row().classes('justify-end mt-3'):
                                                                    ui.button('Close', on_click=lambda: dlg.close()).props('flat dense')
                                                        dlg.open()

                                                    ui.menu_item('Confirm Donation', on_click=_on_confirm).props('icon=check_circle')
                                                    ...
                                                    ui.menu_item('View Responses', on_click=_on_view).props('icon=reply')
                ...
                async def load_hospital_info():
                    ...
                    if not data:
                        ...
                        if hospital_id:
                            def _get_by_id():
                                return requests.get(f"{base_url}/hospitals/{hospital_id}", headers={"Authorization": f"Bearer {token}"})
                    ...
                    if data:
                        ...
                        name = hosp.get('hospital_name') or hosp.get('name') or hosp.get('hospitalName')
                        address = hosp.get('location_address') or hosp.get('address') or hosp.get('location')
                        ...
                        if address:
                            ...
                            hospital_map.clear()
                            with hospital_map:
                                ui.html(iframe_html, sanitize=False).classes("w-full h-50 rounded-md overflow-hidden")
